...
class TreeHelper extends AppHelper {
	 ...
	 * @return string
	 */
	protected function menuIterator($array, $loop=0){
		
		$str = '';
		$is_selected = false;
                $menupop = $loop==0?'menupop':'';
		foreach($array as $var){
			...
			if($continue == true){
				...
				if(!empty($var['children'])){
					$sub .= '<ul>';
					$sub .= $this->menuIterator($var['children'], $loop+1);
					$sub .= '</ul>';	
				}
				...
				$str .= '<li class="' . $menupop . ' ' . $selected . '" ' . $default_selected . ' '. $loop .'>';
					$str .= '<a  href="' . $this->webroot . $url['url'] . '" ' . $url['target'] . '>' . $name . '</a>';
					$str .= $sub;
				$str .= '</li>';
