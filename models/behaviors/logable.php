	...
	function findUserActions(&$Model, $user_id, $params = array()) {
		...
		foreach ($data as $key => $row) {$one = $row['Log'];
			...
			if (isset($one['model']) && isset($one['action']) && isset($one['change']) && isset($one['model_id'])) {
				 if ($one['action'] == 'edit') {
				 	$result[$key]['Log']['event'] .= __(' edited ',true).$one['change'].__(' of ',true).low($one['model']).'(id '.$one['model_id'].')';
				 	//	' at '.$one['created']; 
				 } elseif ($one['action'] == 'add') {
				 	$result[$key]['Log']['event'] .= __(' added a ',true).low($one['model']).'(id '.$one['model_id'].')';
				 } elseif ($one['action'] == 'delete') {
				 	$result[$key]['Log']['event'] .= __(' deleted the ',true).low($one['model']).'(id '.$one['model_id'].')';
				 }
					 	
			} elseif ( isset($one['model']) && isset($one['action'])  && isset($one['model_id']) ) { // have model,model_id and action
                 if ($one['action'] == 'edit') {
				 	$result[$key]['Log']['event'] .= __(' edited ',true).low($one['model']).'(id '.$one['model_id'].')';
				 	//	' at '.$one['created']; 
				 } elseif ($one['action'] == 'add') {
				 	$result[$key]['Log']['event'] .= __(' added a ',true).low($one['model']).'(id '.$one['model_id'].')';
				 } elseif ($one['action'] == 'delete') {
    ...
    function _saveLog(&$Model, $logData, $title = null) {  
        ...
        if (isset($this->Log->_schema['description'])) {
        	if ($this->user && $this->UserModel) {
        		$logData['Log']['description'] .= __(' by ',true).$this->settings[$Model->alias]['userModel'].' "'.
        		...
        		$logData['Log']['description'] .= __(' by System',true);
        	}
